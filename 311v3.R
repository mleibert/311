...
length(unique(calls$block))


for(i in 1:nrow(blktract)){ calls[which(calls$block == 
	blktract[i,1] ) ,]$tract<-blktract[i,2]}
...
calls$ID<-1:nrow(calls)

calls[which(calls$block > 200338018072),]$tract
vvv<-unique(calls[which(calls$block > 200338018072),]$block)
for(i in 1:length(vvv)){ calls[which(calls$block == vvv[i]),]$tract<-
	paste0("MD",i)}
...
k=1
addon<-unique(calls[which( calls$block %in% dat$block   == 
	F), c(  (ncol(calls)-2) ,(ncol(calls)-1)	)	] )
names(addon)[2]<-"Geography"
addon$Commuters<-0; addon$Drivers<-0;addon$Pdrive<-0
dat <-rbind(dat,addon)
...
addon<-unique(calls[which( calls$block %in% census[[k]]$block   == 
	F),c(1,49)] )
addon$Income<-0
census[[k]] <-rbind(census[[k]],addon)
...
addon<-unique(calls[which( calls$Geography %in% census[[k]]$tract == 
	F),c(1,49)] )
head(census[[k]] ,3);head(addon,3)
vvv<-ncol(census[[k]])-ncol(addon);vvv
for(i in 1:vvv){ addon[,2+i]<-0 }
names(addon)<-names(census[[k]])
census[[k]] <-rbind(census[[k]],addon)
...
vvv<-which(names(calls) %in% c("block","Geography"))
addon<-unique(	calls[which( calls$block %in% census[[k]]$block == 
	F),vvv] )
head(census[[k]] ,3);head(addon,3)
vvv<-ncol(census[[k]])-ncol(addon);vvv
